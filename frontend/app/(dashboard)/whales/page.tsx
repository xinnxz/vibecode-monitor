...
// ============================================================
// Whale Tracker — live feed semua WhaleAlert dari kontrak.
//
// Fitur:
// - Live alert list dari useWhaleAlerts
// - Setiap alert: from → to, amount, timestamp, severity badge
// - Volume bars (relative to largest alert in viewport)
// - Stats: total detected, largest whale, average amount
// ============================================================
...
// ——— Severity berdasarkan jumlah STT ———
...
function WhaleCard({ alert, maxAmount, index }: {
  ...
  const barPct = maxAmount > 0n
    ? (Number(alert.amount) / Number(maxAmount)) * 100
    : 0;
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-0 left-0 h-1 bg-current opacity-30 transition-all duration-1000"
        style={{ width: `${barPct}%`, color: sev.color.replace("text-", "") }}
      />
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="shrink-0 text-center">
          ...
          <span className={`text-[10px] font-mono font-bold ${sev.color} px-1 rounded`}>
            {sev.label}
          </span>
        </div>
        ...
        <div className="shrink-0 flex flex-col items-end gap-1">
          <div className="w-2 bg-white/10 rounded-full h-16 overflow-hidden">
            <div
              className={`w-full rounded-full transition-all duration-1000 ${sev.color.replace("text-", "bg-")}`}
              style={{ height: `${barPct}%`, marginTop: "auto" }}
            />
          </div>
...
export default function WhalesPage() {
  ...
  const maxAmount = alerts.length > 0
    ? alerts.reduce((max, a) => a.amount > max ? a.amount : max, 0n)
    : 0n;
    ...
    <div className="p-6 pb-12">
      ...
      <div className="mb-6">
        ...
        <p className="text-white/40 text-sm font-mono ml-11">
          Large STT transfers detected via Somnia Reactivity
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          { label: "Largest", value: maxAmount > 0n ? `${fmtSTT(maxAmount)} STT` : "—", color: "text-yellow-400", icon: "🏆" },
        ].map((stat) => (
        ...
        {alerts.length === 0 ? (
          <div className="glass rounded-xl flex flex-col items-center justify-center py-20">
            ...
            <p className="text-white/20 font-mono text-xs mt-1">
              Deploy WhaleDetector.sol to start detecting
            </p>
          </div>
        ) : (
          <AnimatePresence>
